...
export const Hero: React.FC = () => {
  ...
  return (
    <section ref={containerRef} className="relative pt-24 pb-12 md:pt-32 md:pb-20 lg:pt-56 lg:pb-40 bg-zinc-50 overflow-hidden bg-grid min-h-[100dvh] flex items-center">
      <motion.div style={{ y: blobsY, opacity }} className="absolute inset-0 pointer-events-none">
        <AmbientGroup />
        
        {/* Floating Decorative Elements - Hidden on mobile for performance and clarity */}
        ...
        <AmbientBlobs color="bg-primary" size="w-[300px] h-[300px] md:w-[500px] md:h-[500px]" className="-top-24 -right-24" opacity="opacity-[0.12]" animation="animate-blob-slow" />
        <AmbientBlobs color="bg-primary" size="w-[250px] h-[250px] md:w-[400px] md:h-[400px]" className="top-1/2 -left-48" opacity="opacity-[0.15]" animation="animate-blob" />
        <AmbientBlobs color="bg-primary" size="w-[400px] h-[400px] md:w-[600px] md:h-[600px]" className="-bottom-48 right-1/4" opacity="opacity-[0.10]" animation="animate-blob-spin" />
      </motion.div>

      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-full bg-zinc-200/50 hidden lg:block"></div>
      
      {/* Smooth Transition Fader to next section */}
      <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-zinc-50 via-zinc-50/80 to-transparent z-20 pointer-events-none"></div>
      
      <div className="container mx-auto px-6 relative z-10 py-10 md:py-0">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-24">
          <motion.div style={{ y: textY, opacity }} className="w-full lg:w-1/2 space-y-8 md:space-y-12 text-center lg:text-left">
            <AnimatedText 
              text="We Build Autonomous Systems."
              className="text-4xl md:text-6xl lg:text-7xl font-black leading-[1] md:leading-[0.9] tracking-tighter text-secondary uppercase"
              delay={0.2}
            ...
            <div className="max-w-xl mx-auto lg:mx-0 space-y-10">
              <Reveal direction="up" delay={0.4} duration={1.2}>
                <div className="relative">
                  <div className="absolute -left-8 top-0 bottom-0 w-1 bg-gradient-to-b from-primary to-transparent opacity-50 hidden lg:block"></div>
                  <p className="text-base md:text-lg lg:text-xl text-zinc-600 leading-relaxed font-medium">
              ...
              <div className="flex flex-col space-y-8 items-center lg:items-start">
                <div className="flex flex-col sm:flex-row items-center gap-5">
                  <div className="flex -space-x-3 md:-space-x-4">
                    ...
                    <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-full border-2 md:border-4 border-zinc-50 bg-black flex items-center justify-center text-white text-[8px] md:text-[10px] font-bold shadow-sm">
                      50+
                    </div>
                  </div>
                  <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
                    ...
                    <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-secondary">Trusted by 50+ Global Teams</span>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-8 w-full sm:w-auto">
                  <a href="#contact" className="w-full sm:w-auto bg-primary text-white px-8 md:px-10 py-4 md:py-5 font-black uppercase text-center tracking-[0.2em] text-[10px] hover:bg-black transition-all duration-300 shadow-[0_10px_30px_rgba(32,188,97,0.3)] hover:shadow-none hover:-translate-y-1 flex items-center justify-center gap-3 group rounded-[4px]">
                    <span className="relative z-10">Start Your Project</span>
                    <Icons.ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                  </a>
                  <a href="#services" className="group flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-secondary hover:text-primary transition-all">
                    <div className="w-10 h-10 rounded-[4px] border border-black/10 flex items-center justify-center group-hover:border-primary group-hover:bg-primary/5 transition-all">
          ...
          <motion.div style={{ y: nodeY, opacity }} className="w-full lg:w-1/2 h-[300px] sm:h-[400px] md:h-[500px] lg:h-[600px] relative overflow-visible">
