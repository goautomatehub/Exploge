    ...
    <section ref={containerRef} className="relative pt-24 pb-12 md:pt-32 md:pb-20 lg:pt-56 lg:pb-40 bg-zinc-50 overflow-hidden bg-grid">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-24">
          <motion.div style={{ y: textY, opacity }} className="w-full lg:w-1/2 space-y-12">
            <AnimatedText 
              text="We Build Autonomous Systems."
              className="text-4xl md:text-5xl lg:text-7xl font-black leading-[0.9] tracking-tighter text-secondary uppercase"
              delay={0.2}
            ...
            <div className="max-w-xl space-y-10">
              <Reveal direction="up" delay={0.4} duration={1.2}>
                <div className="relative">
                  <div className="absolute -left-8 top-0 bottom-0 w-1 bg-gradient-to-b from-primary to-transparent opacity-50"></div>
                ...
                <div className="flex items-center gap-5">
                  <div className="flex -space-x-4">
                    {[
                      ...
                      <div key={i} className="relative w-12 h-12 rounded-full border-4 border-zinc-50 overflow-hidden bg-zinc-100 hover:translate-y-[-4px] transition-transform duration-300 shadow-sm">
                        <img src={url} alt="Client" className="w-full h-full object-cover" />
                      </div>
                    ))}
                    <div className="relative w-12 h-12 rounded-full border-4 border-zinc-50 bg-black flex items-center justify-center text-white text-[10px] font-bold shadow-sm">
                      50+
                    </div>
                  </div>
                  <div className="flex flex-col">
                    <div className="flex gap-0.5 mb-1">
                      {[1, 2, 3, 4, 5].map(s => <Icons.Star key={s} className="w-3 h-3 text-yellow-500 fill-current" />)}
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary">Trusted by 50+ Global Teams</span>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row items-center gap-8">
                  <a href="#contact" className="w-full sm:w-auto bg-primary text-white px-10 py-5 font-black uppercase text-center tracking-[0.2em] text-[10px] hover:bg-black transition-all duration-300 shadow-[0_10px_30px_rgba(32,188,97,0.3)] hover:shadow-none hover:-translate-y-1 flex items-center justify-center gap-3 group rounded-[4px]">
